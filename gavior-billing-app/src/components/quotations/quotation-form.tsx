    ...
    <form action={createQuotationAction} className="space-y-6">
        ...
        <div className="ops-form-grid">
          ...
          <label className="ops-field">
            ...
            <input
              ...
              required
              maxLength={180}
              placeholder="e.g. Website Re-architecture & Growth Marketing"
            />
          </label>

          <label className="ops-field ops-field-full">
            Project Objectives & Client Summary
            <textarea
              name="short_summary"
              required
              minLength={10}
              maxLength={4000}
              rows={3}
              placeholder="Detail client business objectives, core problems to solve, deliverable boundaries, and specific expectations."
            />
          </label>
          ...
          <label className="ops-field">
            ...
            <input
              name="timeline"
              maxLength={500}
              placeholder="e.g. 4–6 Weeks (Phased milestone sprints)"
            />
          </label>
      ...
      <section className="ops-panel">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-bold text-gray-900">Commercial Line Items</h2>
            <p className="text-xs text-gray-500">
              Pick standard Gavior plans from the catalog or customize on the fly.
            </p>
          </div>
          <button
            type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mb-2">
                <label className="ops-field md:col-span-3">
                  <span className="flex items-center gap-1">
                    <Layers size={13} className="text-[#1e6a47]" /> Catalog Quick-Select
                  </span>
                  <select
                    value={item.catalog_item_id || ""}
                    onChange={(e) => handleSelectCatalog(index, e.target.value)}
                  >
                    <option value="">Custom Item / Custom Plan</option>
                    {catalog.map((c) => (
                      <option key={c.id} value={c.id}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
                <label className="ops-field md:col-span-4">
                  ...
                  <input
                    ...
                    placeholder="Describe specific features, pages, or deliverables included in this item."
                    required
                  />
                </label>
        ...
        <div className="mt-6 p-4 rounded-xl bg-white border border-[#dde5e0] max-w-sm ml-auto space-y-2 text-sm">
          ...
          <div className="flex justify-between text-base font-extrabold text-[#1e6a47] pt-2 border-t border-gray-200">
      ...
      <div className="flex items-center justify-between pt-2">
        <Link href="/quotations" className="ops-btn ops-btn-secondary">
          Cancel
        </Link>
        <button type="submit" className="ops-btn">
          Create Draft & Proceed to AI Proposal
        </button>
      </div>
    </form>
  );
